...
class VotingClassifier:
    def __init__(self, Linear_list, knn_list, tree_list, Ptree_list):
    ...
    def predict(self, X, y):
        time = 0
        accuracies = []
        for classifier in zip(self.Linear_list, self.knn_list, self.tree_list, self.Ptree_list):
            self.votes = []
            for classify in classifier:
                prediction = classify.predict(X)
                self.votes.append(prediction)

            # 進行投票，選擇得票最多的類別作為最終預測
            result = []
            for i in range(len(X)):
                votes_i = [vote[i] for vote in self.votes]
                most_common = max(set(votes_i), key=votes_i.count)
                result.append(most_common)
            avg_accuracy = np.mean(result == y)
            accuracies.append(avg_accuracy)
            print(f'VotingClassifier of Fold {i + 1} Average Accuracy: {avg_accuracy}')
            time+=1
        # 計算平均準確度
        avg_accuracy = np.mean(accuracies)
        print(f'平均 VotingClassifier 準確度：{avg_accuracy}')
###############################  VotingClassifier  ###############################
